//hacked together version for my dum rotated screen
//;-;
#include <stdint.h>
#include <iostream>
#include <cmath>
...
extern "C" void drawLineUpDown(context *ctx, int32_t y, int32_t x, int32_t l, uint32_t c)
{
    y = (ctx->height - y) - 1;
...
extern "C" void drawLineLeftRight(context *ctx, int32_t y, int32_t x, int32_t l, uint32_t c)
{
    y = (ctx->height - y) - l;
...
extern "C" void drawLine(context *ctx, int32_t y1, int32_t x1, int32_t y2, int32_t x2, uint32_t c)
{
    y1 = (ctx->height - y1) - 1;
    y2 = (ctx->height - y2) - 1;
    ...
    if (abs(dx) < abs(dy))
    {
        ...
        if (dy == 0)
        {
            drawLineLeftRight(ctx, (ctx->width - y1) - 1, x1, dy, c);
            return;
        }
    ...
    else
    {
        ...
        if (dx == 0)
        {
            drawLineUpDown(ctx, (ctx->width - y1) - 1, x1, dx, c);
            return;
        }
